...
public class YouTubeAnalyticsReports {
	...
	/**
	 * Define a global instance of a YoutubeAnalytics object, which will be used to
	 * make YouTube Analytics API requests.
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			youtube = new YouTube.Builder(HTTP_TRANSPORT, JSON_FACTORY, credential)
					.setApplicationName("youtube-analytics-api-report-example").build();
	...
	private static ResultTable executeViewsOverTimeQuery(YouTubeAnalytics analytics, String id) throws IOException {
		...
		return analytics.reports().query("channel==" + id, // channel id
				"2012-01-01", // Start date.
				"2012-01-14", // End date.
				"views,uniques") // Metrics.
				.setDimensions("day").setSort("day").execute();
	}
	...
	private static ResultTable executeTopVideosQuery(YouTubeAnalytics analytics, String id) throws IOException {
		...
		return analytics.reports().query("channel==" + id, // channel id
				...
				.setDimensions("video").setSort("-views").setMaxResults(10).execute();
	}
	...
	private static ResultTable executeDemographicsQuery(YouTubeAnalytics analytics, String id) throws IOException {
		return analytics.reports().query("channel==" + id, // channel id
				...
				.setDimensions("ageGroup,gender").setSort("-viewerPercentage").execute();
	}

	/**
	 * Prints the API response. The channel name is printed along with each column
	 * name and all the data in the rows.
	 *
