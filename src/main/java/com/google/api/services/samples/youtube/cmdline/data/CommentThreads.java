 ...
 *
 * 1. Creating a top-level comments for a video and a channel via
 * "commentThreads.insert" method. 2. Retrieving the top-level comments for a
 * video and a channel via "commentThreads.list" method. 3. Updating an existing
 * comments via "commentThreads.update" method.
 *
...
public class CommentThreads {
	...
	public static void main(String[] args) {
		...
		try {
			...
			CommentThreadListResponse channelCommentsListResponse = youtube.commentThreads().list("snippet")
			...
			if (channelComments.isEmpty()) {
				...
				firstComment.getSnippet().getTopLevelComment().getSnippet().setTextOriginal("updated");
				CommentThread channelCommentUpdateResponse = youtube.commentThreads().update("snippet", firstComment)
						.execute();
				// Print information from the API response.
