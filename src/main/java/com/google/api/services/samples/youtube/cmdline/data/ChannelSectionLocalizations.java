 ...
 * for a channel section via "channelSections.update" method. 2. Getting the
 * localized metadata for a channel section in a selected language using the
 * "channelSections.list" method and setting the "hl" parameter. 3. Listing the
 * localized metadata for a channel section using the "channelSections.list"
 * method and including "localizations" in the "part" parameter.
 *
...
public class ChannelSectionLocalizations {
	 ...
	 *
	 * @param channelSectionId The id parameter specifies the channel section ID for
	 *                         the resource that is being updated.
	 * @param defaultLanguage  The language of the channel section's default
	 *                         metadata
	 * @param language         The language of the localized metadata
	 * @param title            The localized title to be set
	 * @throws IOException
	 */
	private static void setChannelSectionLocalization(String channelSectionId, String defaultLanguage, String language,
			String title) throws IOException {
		// Call the YouTube Data API's channelSections.list method to retrieve channel
		// sections.
		...
		// not contain a channel section, then the specified channel section ID was not
		// found.
		...
		// Ensure that a value is set for the resource's snippet.defaultLanguage
		// property.
		channelSection.getSnippet().setDefaultLanguage(defaultLanguage);

		// Preserve any localizations already associated with the channel section. If
		// the
		// channel section does not have any localizations, create a new array. Append
		// the
		// provided localization to the list of localizations associated with the
		// channel section.
		...
		// Update the channel section resource by calling the channelSections.update()
		// method.
		...
		System.out.println("  - Default Language: " + channelSectionResponse.getSnippet().getDefaultLanguage());
		System.out.println(
				"  - Title(" + language + "): " + channelSectionResponse.getLocalizations().get(language).getTitle());
		System.out.println("\n-------------------------------------------------------------\n");
	}

	/**
	 * Returns localized metadata for a channel section in a selected language. If
	 * the localized text is not available in the requested language, this method
	 * will return text in the default language.
	 *
	 * @param channelSectionId The id parameter specifies the channel section ID for
	 *                         the resource that is being updated.
	 * @param language         The language of the localized metadata
	 * @throws IOException
	 */
	private static void getChannelSectionLocalization(String channelSectionId, String language) throws IOException {
		// Call the YouTube Data API's channelSections.list method to retrieve channel
		// sections.
		...
		System.out
				.println("  - Title(" + language + "): " + channelSection.getLocalizations().get(language).getTitle());
		System.out.println("\n-------------------------------------------------------------\n");
	}
	...
	private static String getDefaultLanguage() throws IOException {
		...
		System.out.println("You chose " + defaultlanguage + " as the language for the resource's default metadata.");
		return defaultlanguage;
	}

	/*
	 * Prompt the user to enter a language for the localized metadata. Then return
	 * the language.
	 */
